...
completed: "2026-06-15T19:53:32Z"
...
- Project DoD applies (`docs/dod.md`): frozen slugs (a slug rename breaks deterministic UUID5 and is its own spec); closed `RecurrenceKind` enum (new kinds = new spec); `bborbe/errors` 3-arg `Wrap(ctx, err, msg)` with no `fmt.Errorf`; no `context.Background()` in business logic; Ginkgo v2 / Gomega for tests.
...
- [ ] The publisher's switch on `RecurrenceKind` has a `default` arm that returns a wrapped error (via `errors.Wrap` / `errors.Wrapf`) when the kind is not mapped to a period-token form; a Ginkgo test constructs a `TaskDefinition` whose `Recurrence` value falls outside the closed enum (via `RecurrenceKind(99)` or equivalent) and asserts the returned error is non-nil and contains the offending kind in its message — evidence: Go test, exit code 0
