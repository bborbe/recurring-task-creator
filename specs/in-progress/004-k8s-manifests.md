...
After this work, running `BRANCH=dev make buca` from the deployment worktree builds the image, pushes it to the registry, and triggers a rolling update of a single-replica StatefulSet in the `dev` namespace whose pod (a) reads `KAFKA_BROKERS`, `STAGE=dev`, `SENTRY_DSN`, `SENTRY_PROXY`, and `TZ=Europe/Berlin` from its own env, (b) exposes HTTP on `:9090` for `/healthz`, `/readiness`, `/metrics`, `/setloglevel/{level}`, and `/trigger` (Spec 4), (c) is reachable inside the cluster at `recurring-task-creator.dev:9090` and from outside at `https://recurring-task-creator.dev.quant.benjamin-borbe.de/`, (d) is scraped by the cluster's Prometheus via the `prometheus.io/scrape: "true"` pod annotation, (e) is authenticated to Kafka via mTLS using a strimzi `KafkaUser` named `dev-recurring-task-creator` (and `prod-recurring-task-creator` for the prod deploy), and (f) runs as non-root with a read-only root filesystem. The same flow works for `BRANCH=prod` against the `prod` namespace, with no per-namespace YAML branching beyond the `NAMESPACE` template variable already supported by `Makefile.k8s`.
...
- Do NOT remove the `imagePullSecrets` `docker-quant` reference — the registry requires it.
...
12. The `Ingress` named `recurring-task-creator` routes the host `recurring-task-creator.{{ "NAMESPACE" | env }}.quant.benjamin-borbe.de` to the service's `http` port via traefik, with TLS terminated by `tls-quant`.
...
20. Rendered manifests for `NAMESPACE=dev`, `BRANCH=dev`, `DOCKER_REGISTRY=docker.benjamin-borbe.de`, `STAGE=dev` pass `kubeval` (or `kubeconform`) against the cluster's API versions. Same for `NAMESPACE=prod`.
...
- The host pattern `recurring-task-creator.<NAMESPACE>.quant.benjamin-borbe.de` MUST match the cluster's traefik TLS SAN list (covered by `tls-quant`); deviation breaks TLS.
...
- The Ingress class MUST be `traefik` and the TLS secret MUST be `tls-quant` — cluster convention.
...
| `tls-quant` secret expires or is rotated mid-deploy | Existing TLS termination continues for the cached cert; new connections may fail after the cert's actual expiry | Cluster operator's TLS rotation playbook handles this — out of scope for this service | `curl -v https://recurring-task-creator.dev.quant.benjamin-borbe.de/healthz` shows TLS error | Reversible at the cluster level | N/A |
...
- [ ] `imagePullSecrets` lists `docker-quant` (or whichever name the parent module's `Makefile.env` uses for the quant registry — verify against `~/Documents/workspaces/maintainer/watcher/github-pr/k8s/maintainer-watcher-github-pr-sts.yaml`) — evidence: `yq '.spec.template.spec.imagePullSecrets[0].name' k8s/recurring-task-creator-sts.yaml` returns the convention name.
...
- [ ] `k8s/recurring-task-creator-ing.yaml` routes host `recurring-task-creator.{{ "NAMESPACE" | env }}.quant.benjamin-borbe.de` to service `recurring-task-creator` port `http`, with TLS via `tls-quant` and `ingressClassName: traefik` — evidence: `grep -nE 'recurring-task-creator\.\{\{ "NAMESPACE" \| env \}\}\.quant\.benjamin-borbe\.de' k8s/recurring-task-creator-ing.yaml` returns at least two matches (host rule + TLS host); `grep -nE 'tls-quant|ingressClassName: .traefik.' k8s/recurring-task-creator-ing.yaml` returns at least two matches.
