...
7. `k8s/apis/task.benjamin-borbe.de/v1/testdata/example.yaml` contains a valid `Schedule` matching the design-pin example (`weekly-review`, namespace `default`, vault `default`, etc.). `k8s/apis/task.benjamin-borbe.de/v1/example_test.go` reads the file, unmarshals it via `sigs.k8s.io/yaml` into a `Schedule` struct, and asserts every field round-trips (Ginkgo `It`).
