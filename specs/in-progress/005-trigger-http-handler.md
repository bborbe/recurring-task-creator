...
verifying: "2026-06-14T13:03:33Z"
...
- `/trigger` is intentionally unauthenticated. The Spec 5 `Service` is cluster-internal (ClusterIP) and the Spec 5 Ingress, per its host pattern, may expose `/trigger` to the public internet behind the `example.com` TLS gateway. The trust boundary is therefore the cluster perimeter plus the TLS gateway's coarse-grained authentication. Even if an external actor reaches `/trigger`, the worst case is a replay of the day's already-deterministic-UUID5 publishes — the controller dedups successful re-creates. Pathological abuse (1000 concurrent requests for the same date) costs Kafka producer capacity but produces zero duplicate vault files.
