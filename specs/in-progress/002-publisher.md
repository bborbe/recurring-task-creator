...
7. The constructed `Frontmatter` is exactly: `assignee: bborbe`, `status: in_progress`, `page_type: task`, `goals: ["[[Migrate Personal Workflow from Atlassian to Obsidian]]"]`, `priority: 2`, `recurring: <kind>` where `<kind>` is the string value of `def.Recurrence` (`daily | weekly | monthly | quarterly | yearly`). No other keys are set by the publisher.
...
- [ ] `cmd.Frontmatter["assignee"] == "bborbe"`, `cmd.Frontmatter["status"] == "in_progress"`, `cmd.Frontmatter["page_type"] == "task"`, `cmd.Frontmatter["priority"] == 2`, `cmd.Frontmatter["goals"]` equals `[]string{"[[Migrate Personal Workflow from Atlassian to Obsidian]]"}` (or its `[]interface{}` equivalent if the type demands it) — evidence: Ginkgo assertions on each key.
