...
- feat: Wire the cleanup cron into the existing publisher binary as a third `run.Func` in `run.CancelOnFirstFinish` (alongside the HTTP server and the publisher tick); new `GIT_REST_URL` / `GATEWAY_SECRET` / `CLEANUP_CRON` env vars on `k8s/recurring-task-creator-sts.yaml`; no separate binary, STS, or image.
