...
- Stamps every command's frontmatter with the fixed migration shape (`assignee: bborbe`, `status: in_progress`, `page_type: task`, `goals: ["[[Migrate Personal Workflow from Atlassian to Obsidian]]"]`, `priority: 2`, `recurring: <kind>`); no other keys, no override knob, no per-task opt-out.
...
Expect(cmd.Frontmatter).To(HaveKeyWithValue("goals", []interface{}{"[[Migrate Personal Workflow from Atlassian to Obsidian]]"}))
